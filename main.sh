...
for i in `cat hosts`; do 
    ...
    if [ -z "$setupcompleted" ]; then
		...
		echo "copy files to server"
       		scp -i /home/ubuntu/.ssh/id_rsa $i.* ubuntu@34.203.245.252:/tmp/                               #Ip 
		...
		echo "move conf and html file"
       		ssh -t ubuntu@34.203.245.252  sudo mv /tmp/$i.conf /etc/apache2/sites-enabled/$i.conf
       		ssh -t ubuntu@34.203.245.252  sudo mv /tmp/$i.html /var/www/html/$i/index.html
       		##restart apache 
       		echo "reload apache"
       		ssh -t ubuntu@34.203.245.252   sudo systemctl reload apache2
